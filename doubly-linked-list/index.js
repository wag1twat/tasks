    ...
    try {
        ...
        if (testResults.length === 0) {
            ...
            testResults.forEach((result, index) => {
                content += `${index + 1}. ${result.label}: ${result.time} ${result.unit}\n`;
            });
            content += '-'.repeat(50) + '\n\n';
            ...
            if (nativeResults.length > 0 && customResults.length > 0) {
                const nativeAvg =
                    nativeResults.reduce((sum, r) => sum + r.time, 0) / nativeResults.length;
                // не учтены накладные расходы на преобразование массива в двусвязный список
                const customAvg =
                    customResults.reduce((sum, r) => sum + r.time, 0) / customResults.length;
...
for (var i = 0; i < 100; i++) {
    test('native', () => array.find((item) => item === 500000 || item === 400000 || item === 1));
    test('custom', () =>
        doublyLinkedList.find((item) => item === 500000 || item === 400000 || item === 1));
}
